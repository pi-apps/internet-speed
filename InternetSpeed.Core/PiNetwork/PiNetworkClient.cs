...
using static InternetSpeed.DataLayer.PiModels.PaymentDataModel;
...
namespace InternetSpeed.Core.PiNetwork
{
    public class PiNetworkClient
    {
        ...
        public async Task<List<PaymentDto>> GetIncompleteServerPayments()
        {
            try
            {
                ...
                if (response.IsSuccessful)
                {
                    ...
                    if (payments != null && payments.IncompletePayments != null &&
                        payments.IncompletePayments.Count > 0)
        ...
        public async Task<SubmitTransactionResponse> SendNativeAssets(string network, string seed, TransactionData data,
            uint fee = 100000)
        {
        ...
        public async Task<SubmitTransactionResponse> SendNativeAssets(string network, KeyPair sourceKeypair,
            TransactionData data, uint fee = 100000)
        {
            //Set network and server
            Server server = await GetServerAsync(network);
            ...
            try
            {
                ...
                MemoText memo = new MemoText(string.IsNullOrEmpty(Identifier)
                    ? $""
                    : Identifier.Substring(0, Math.Min(Identifier.Length, 28)));
